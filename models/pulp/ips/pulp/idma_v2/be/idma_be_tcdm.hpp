 ...
 *
 * io_v2 port of pulp/idma/be/idma_be_tcdm.{hpp,cpp}. Same model: a direct
 * interface to a local memory, one line-sized request at a time, blocked until
 * that request is done.
 *
 * The TCDM port sits on the non-beat plane (IoV2SingleReq): the local
 * interconnect answers every line inline with IO_REQ_DONE and annotates its
 * cost on the request, which is exactly what this back-end's timing model
 * consumes. An asynchronous (GRANTED) or back-pressured (DENIED) answer is
 * rejected loudly, as in the v1 model — the DMA is modelled as always winning
 * arbitration on the TCDM side.
 */
...
private:
    ...
    // Send one line request to the TCDM. Returns false when the interconnect
    // refused it (IO_REQ_DENIED) — the caller then holds the line and it is
    // re-sent from ico_retry(). On success *latency gets the reported cost.
    bool send_line(vp::IoReq *req, uint64_t base, uint64_t size, int64_t *latency);
    // Post-send bookkeeping, shared between the first attempt and the re-send
    // from ico_retry().
    void write_line_completed(uint64_t size, int64_t latency);
    void read_line_completed(uint64_t size, int64_t latency);

    // A line the interconnect refused, held until the retry re-sends it. The
    // request object (this->req) still carries addr / size / data.
    enum DeniedKind { DENIED_NONE, DENIED_WRITE, DENIED_READ };
    DeniedKind denied_kind = DENIED_NONE;
    uint64_t   denied_base = 0;
    uint64_t   denied_size = 0;
    ...
    IdmaBeProducer *be;
    // TCDM interface where requests can be posted. ico_retry() re-sends a
    // refused line; ico_resp() only exists so that an interconnect answering
    // asynchronously fails with a clear message instead of being mishandled.
    static vp::IoRespAck ico_resp(vp::Block *__this, vp::IoReq *req);
    static void ico_retry(vp::Block *__this, vp::IoRetryChannel channel);
    vp::IoMaster ico_itf{&IDmaBeTcdm::ico_retry, &IDmaBeTcdm::ico_resp};

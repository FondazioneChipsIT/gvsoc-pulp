...
from pulp.chips.magia_v4.ctrl_core.hierarchical_cache import CV32_Hierarchical_cache

from pulp.chips.magia_v4.arch import *
from pulp.chips.magia_v4.ctrl_core.core import CV32CtrlCore
from pulp.chips.magia_v4.pulp_core.core import CV32PulpCore
from pulp.light_redmule.light_redmule import LightRedmule
from pulp.idma.snitch_dma import SnitchDma
from pulp.chips.magia_v4.fractal_sync_mm_ctrl.fractal_sync_mm_ctrl import FSync_mm_ctrl
from pulp.chips.magia_v4.idma_mm_ctrl.idma_mm_ctrl import iDMA_mm_ctrl
from pulp.chips.magia_v4.cluster_regs.cluster_regs import ClusterRegs
...
class MagiaTileTcdm(gvsoc.systree.Component):
    def __init__(self, parent, name, tree, parser):
        ...
        # Port map, mirroring hci_core_if[] in magia_tile.sv:
        #   0                    OBI xbar (external/mesh L1 traffic, stack, reserved)
        #   1 .. nb_pulp_cores   one dedicated port per PULP cluster core
        #   last                 the control core's own dedicated port
        # Both the control core and the cluster cores reach L1 straight from their
        # data demux (core_data_demux.sv), bypassing the OBI xbar. In RTL all these
        # ports share a single hci_interconnect, hence one interleaver here too.
        L1_masters = 1 + (tree.nb_pulp_cores if MagiaArch.PULP_ENABLE else 0) + 1
        self.ctrl_port = L1_masters - 1
    ...
    def i_CTRL_INPUT(self) -> gvsoc.systree.SlaveItf:
        """Dedicated L1 port of the control core: the last master, as in
        magia_tile.sv where it is hci_core_if[NumHciCore-1]."""
        return gvsoc.systree.SlaveItf(self, f'L1_input_{self.ctrl_port}', signature='io')
...
class MagiaV4Tile(gvsoc.systree.Component):
    def ev_unit_config(self,tree):
        # Same config whether or not the PULP cluster is present: this Event Unit
        # serves the control core only (NB_CORES=1 in magia_tile.sv). The cluster
        # has its own instance, see cluster_ev_unit_config().
        event_unit_json = {
            "event_unit": {
                "version": "4",
                "mapping": {
    ...
    def cluster_ev_unit_config(self, tree):
        """Config of the cluster-private Event Unit (magia_cluster_wrap.sv).

        Same IP as the control-core one, instantiated with NB_CORES = number of
        cluster cores and the full barrier/mutex/dispatch resources. Two views of
        the same unit are exposed, exactly as in RTL: the per-core demux
        (eu_direct_link) window at CLUSTER_EU_DIRECT, which each core reaches
        through its own data demux, and the memory-mapped (speriph) window at
        CLUSTER_EU, reached from the OBI xbar. The 'mapping' below is the latter.

        The event positions are the event_unit_flex ones (cluster_event_map.sv,
        gen_cluster_map): barrier 16, mutex 17, dispatch 18 and the software
        events at bits [11:4], i.e. sw_base 4.
        """
        cluster_event_unit_json = {
            "cluster_event_unit": {
                "version": "4",
                "mapping": {
                    "base": MagiaArch.CLUSTER_EU_ADDR_START,
                    "size": MagiaArch.CLUSTER_EU_SIZE,
                    "remove_offset": MagiaArch.CLUSTER_EU_ADDR_START
                },
                "config": {
                    "nb_core": tree.nb_pulp_cores,
                    "properties": {
                        "dispatch": {"size": 8},
                        "mutex": {"nb_mutexes": 1},
                        "barriers": {"nb_barriers": tree.nb_pulp_cores},
                        "soc_event": {"nb_fifo_events": 8, "fifo_event": 8},
                        "events": {
                            "barrier": 16,
                            "mutex": 17,
        ...
        json_data = json.dumps(cluster_event_unit_json)
        return json.loads(json_data)
    ...
    def __init__(self, parent, name, tree, parser, tid: int=0):
        ...
        if MagiaArch.PULP_ENABLE:
            ...
            # Instruction cache (from snitch cluster model)
            pulp_i_cache = CV32_Hierarchical_cache(self, f'tile-{tid}-pulp-icache', nb_cores=tree.nb_pulp_cores, has_cc=0, l1_line_size_bits=4)

            # Cluster-private Event Unit
            self.add_properties(self.cluster_ev_unit_config(tree))
            cluster_event_unit = Event_unit(self, f'tile-{tid}-cluster-event-unit',
                                            self.get_property('cluster_event_unit/config'))

            # Per-core data demux (core_data_demux.sv): the L1 window takes the
            # dedicated TCDM port, the cluster EU demux window takes that core's
            # Event Unit slice, everything else falls through to the OBI xbar.
            pulp_demux = []
            for pulp_id in range(0,tree.nb_pulp_cores):
                pulp_demux.append(router.Router(self, f'tile-{tid}-pulp-{pulp_id}-demux',
                    bandwidth=4, latency=MagiaDSE.TILE_CORE_DEMUX_LATENCY,
                    synchronous=MagiaDSE.TILE_CORE_DEMUX_SYNC))
        ...
        # Control core data demux (core_data_demux.sv, same module the cluster cores
        # use): L1 window -> dedicated TCDM port, tile Event Unit window -> Event
        # Unit directly, everything else -> OBI xbar.
        core_demux = router.Router(self, f'tile-{tid}-core-demux',bandwidth=4,
                                   latency=MagiaDSE.TILE_CORE_DEMUX_LATENCY,
                                   synchronous=MagiaDSE.TILE_CORE_DEMUX_SYNC)
        ...
        # Event Unit
        self.add_properties(self.ev_unit_config(tree))
        ...
        self.__o_LOADER(obi_xbar.i_INPUT()) #FIXME
        ...
        if MagiaArch.PULP_ENABLE:
            # Bind pulp cluster cores data --> per-core demux, then:
            #   L1 window            -> dedicated TCDM port (bypasses the OBI xbar)
            #   cluster EU demux win -> this core's Event Unit slice
            #   anything else        -> OBI xbar (stack, reserved, peripherals, L2)
            for pulp_id in range(0,tree.nb_pulp_cores):
                pulp_cores[pulp_id].o_DATA(pulp_demux[pulp_id].i_INPUT())
                # o_DATA_DEBUG not present on the iss_v2 CV32E40P core (same as
                # the Spatz iss_v2 core above); the debug data master is unused.
                # pulp_cores[pulp_id].o_DATA_DEBUG(pulp_demux[pulp_id].i_INPUT())

                pulp_demux[pulp_id].o_MAP(l1_tcdm.i_INPUT(1 + pulp_id), name='l1-mem-local',
                ...
                pulp_demux[pulp_id].add_mapping('cluster_event_unit_direct',
                        base=MagiaArch.CLUSTER_EU_DIRECT_START,
                        size=MagiaArch.CLUSTER_EU_DIRECT_SIZE,
                        remove_offset=MagiaArch.CLUSTER_EU_DIRECT_START)
                self.bind(pulp_demux[pulp_id], 'cluster_event_unit_direct',
                          cluster_event_unit, f'demux_in_{pulp_id}')

                # Default entry (base and size left at 0): everything else
                pulp_demux[pulp_id].o_MAP(obi_xbar.i_INPUT(), name='obi-xbar')
            ...
            # Bind: pulp cluster icache -> tile interconnect
            pulp_i_cache.o_REFILL(tile_xbar.i_INPUT())
            ...
            # Bind: cluster event unit <-> cluster cores. Same handshake as the
            # control core: vectored irq_req/irq_ack plus the clock gating the EU
            # applies while a core sleeps on cv.elw.
            for pulp_id in range(0,tree.nb_pulp_cores):
                self.bind(cluster_event_unit, f'clock_{pulp_id}', pulp_cores[pulp_id], 'clock')
                self.bind(pulp_cores[pulp_id], 'irq_ack', cluster_event_unit, f'irq_ack_{pulp_id}')
                self.bind(cluster_event_unit, f'irq_req_{pulp_id}', pulp_cores[pulp_id], 'irq_req')

            # Dispatch doorbell: PULP_START rings core 0 only, as EU_OTHER_CLUSTER_START
            self.bind(cluster_regs, 'pulp_start_irq', cluster_event_unit, 'in_event_13_pe_0')

            # Bind obi xbar so that it can reach the memory-mapped view of the cluster EU
            obi_xbar.add_mapping('cluster_event_unit', **self.get_property('cluster_event_unit/mapping'))
            self.bind(obi_xbar, 'cluster_event_unit', cluster_event_unit, 'input')

        # Bind: cv32 core data -> its own data demux, then:
        #   L1 window        -> dedicated TCDM port (bypasses the OBI xbar)
        #   tile EU window   -> Event Unit directly (bypasses the OBI xbar)
        #   anything else    -> OBI xbar (stack, reserved, peripherals, remote L1, L2)
        core_cv32.o_DATA(core_demux.i_INPUT())
        # o_DATA_DEBUG not present on the iss_v2 CV32E40P core; debug data
        # master unused (same as the Spatz iss_v2 core).
        # core_cv32.o_DATA_DEBUG(core_demux.i_INPUT())

        # The control core's dedicated L1 port is the last interleaver master, as in
        # magia_tile.sv (hci_core_if[NumHciCore-1]).
        core_demux.o_MAP(l1_tcdm.i_CTRL_INPUT(), name='l1-mem-local',
                    base=MagiaArch.L1_ADDR_START+(tid*MagiaArch.L1_TILE_OFFSET),
                    size=MagiaArch.L1_SIZE, rm_base=False,
                    remove_offset=(tid*MagiaArch.L1_TILE_OFFSET))

        # The control core reaches its Event Unit without crossing the OBI xbar. In
        # RTL this branch drives the EU's eu_direct link (per-core demux layout);
        # here it lands on the same memory-mapped port the xbar uses, which is the
        # layout event_unit_utils.h actually addresses (barrier 0x400, sw 0x600).
        core_demux.add_mapping('event_unit', **self.get_property('event_unit/mapping'))
        self.bind(core_demux, 'event_unit', event_unit, 'input')

        # Default entry (base and size left at 0): everything else
        core_demux.o_MAP(obi_xbar.i_INPUT(), name='obi-xbar')
        ...
        # Bind: cv32 icache -> tile interconnect
        ...
        if MagiaArch.SPATZ_ENABLE or MagiaArch.PULP_ENABLE:
            obi_xbar.o_MAP(cluster_regs.i_INPUT(), name="cluster-regs",
    ...
    def i_PULP_ENTRY(self) -> gvsoc.systree.SlaveItf:
        return gvsoc.systree.SlaveItf(self, 'pulp_entry', signature='wire<uint64_t>')
    
    def __o_PULP_ENTRY(self, itf: gvsoc.systree.SlaveItf):
        self.itf_bind('pulp_entry', itf, signature='wire<uint64_t>', composite_bind=True)

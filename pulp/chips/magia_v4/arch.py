...
class MagiaArch:
    ...
    # Cluster event unit: two views of the same unit, as in magia_event_unit.sv.
    # CLUSTER_EU_DIRECT_* is the per-core demux (eu_direct_link) window, reached
    # by each cluster core through its own data demux; CLUSTER_EU_* is the
    # memory-mapped (speriph) window, reached from the OBI xbar.
    CLUSTER_EU_DIRECT_START = CLUSTER_CTRL_END + 1
    CLUSTER_EU_DIRECT_SIZE  = 0x0000_0FFF
    CLUSTER_EU_DIRECT_END   = CLUSTER_EU_DIRECT_START + CLUSTER_EU_DIRECT_SIZE
    CLUSTER_EU_ADDR_START   = CLUSTER_EU_DIRECT_END + 1
    CLUSTER_EU_SIZE         = 0x0000_0FFF
    CLUSTER_EU_ADDR_END     = CLUSTER_EU_ADDR_START + CLUSTER_EU_SIZE
    RESERVED_ADDR_START     = CLUSTER_EU_ADDR_END + 1
    RESERVED_SIZE           = 0x0000_C7FF
...
class MagiaTree(Tree):
    ...
    *name*, not to the run command line:
        ...
        make TARGETS="magia_v4:n_tiles_x=2,n_tiles_y=2" build
        gvrun --target=magia_v4:n_tiles_x=2,n_tiles_y=2 --param binary=<elf> run
    ...
    --attr magia_v4/n_tiles_x=... is now rejected instead of silently
    mismatching.

    spatz_romfile stays a plain run-time attribute (--attr
    magia_v4/spatz_romfile=<path>) on purpose: it is a stim_file path that
...
class MagiaDSE:
    ...
    # Per-core data demux (core_data_demux.sv), used by both the control core
    # and every PULP cluster core: combinational in RTL
    TILE_CORE_DEMUX_LATENCY     = 0
    TILE_CORE_DEMUX_SYNC        = True

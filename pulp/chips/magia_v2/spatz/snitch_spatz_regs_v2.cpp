 ...
 *
 * io_v2 port of snitch_spatz_regs.cpp. Internal logic untouched; only the
 * memory-mapped slave port moves to the v2 IO protocol
 * (vp/itf/io_v2.hpp):
 *   - the request handler is attached through the port constructor instead of
 *     set_req_meth(),
 *   - the port answers inline (IoV2Sync contract): IO_REQ_DONE, with
 *     IO_RESP_INVALID on the response-status sideband where v1 returned
 *     IO_REQ_INVALID.
 */
...
protected:
    ...
    vp::IoSlave         input_itf{&SnitchSpatzRegs::req};

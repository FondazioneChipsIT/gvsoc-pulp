...
class CV32_Hierarchical_cache(gvsoc.systree.Component):
    """io_v2 sibling of
    :class:`pulp.chips.magia_v2.cv32.hierarchical_cache.CV32_Hierarchical_cache`.

    Same hierarchy — one private L0 in front of one shared L1, both refilling
    upwards — rebuilt on ``cache.cache_v4`` (the io_v2 cache leaf).

    Two structural differences, both behaviour-preserving:

    - **Geometry is expressed in bytes.** cache_v4 takes ``size`` / ``line_size``
      / ``ways`` where the v1 leaf took ``nb_sets_bits`` / ``nb_ways_bits`` /
      ``line_size_bits``. The values below are the exact translation of the v1
      ones (L0: 32 sets x 1 way x 16 B = 512 B; L1: 32 sets x 32 ways x 16 B =
      16 KiB).
    - **No L1 interleaver.** With ``nb_l1_banks == 1`` the v1
      ``interco.interleaver`` was a pure pass-through (bank mask 0, offset
      unchanged), so the L0 refill port binds the L1 input directly instead of
      instantiating an io_v2 interleaver that would do nothing.

    The refill-latency knob still comes from ``MagiaDSE.TILE_ICACHE_REFILL_LATENCY``
    (taken from magia_v2's own arch.py rather than magia's, as in the v1 file;
    ...
    def __init__(self, parent: gvsoc.systree.Component, name: str, nb_cores: int=0, has_cc: int=0, l1_line_size_bits: int=0):
        ...
        l0_line_size = 1 << 4           # line_size_bits = 4
        l0_sets      = 1 << 5           # nb_sets_bits   = 5
        l0_ways      = 1 << 0           # nb_ways_bits   = 0
        l1_line_size = 1 << 4           # line_size_bits = 4
        l1_sets      = 1 << 5           # nb_sets_bits   = 5
        l1_ways      = 1 << 5           # nb_ways_bits   = 5
        ...
        for i in range(0, nb_l1_banks):
            l1_caches.append(Cache(self, 'l1_bank%d' % i, config=CacheConfig(
                ...
                ways=l1_ways, refill_latency=MagiaDSE.TILE_ICACHE_REFILL_LATENCY,
                enabled=True)))
